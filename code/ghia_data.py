"""
Benchmark data from Ghia, Ghia & Shin (1982), J. Comput. Phys. 48, 387-411,
"High-Re solutions for incompressible flow using the Navier-Stokes equations
and a multigrid method." DOI: 10.1016/0021-9991(82)90058-4

Table I: u-velocity along the vertical centerline (x = 0.5), at 17 y-stations.
Table II: v-velocity along the horizontal centerline (y = 0.5), at 17 x-stations.

These are the canonical, widely reproduced reference values used for
verification of lid-driven cavity solvers.
"""

import numpy as np

# y-coordinates for the u-velocity profile (Table I), from top to bottom.
GHIA_Y = np.array([
    1.0000, 0.9766, 0.9688, 0.9609, 0.9531, 0.8516, 0.7344, 0.6172,
    0.5000, 0.4531, 0.2813, 0.1719, 0.1016, 0.0703, 0.0625, 0.0547, 0.0000
])

# u along vertical centerline x=0.5
GHIA_U = {
    100: np.array([
        1.00000, 0.84123, 0.78871, 0.73722, 0.68717, 0.23151, 0.00332,
        -0.13641, -0.20581, -0.21090, -0.15662, -0.10150, -0.06434,
        -0.04775, -0.04192, -0.03717, 0.00000
    ]),
    400: np.array([
        1.00000, 0.75837, 0.68439, 0.61756, 0.55892, 0.29093, 0.16256,
        0.02135, -0.11477, -0.17119, -0.32726, -0.24299, -0.14612,
        -0.10338, -0.09266, -0.08186, 0.00000
    ]),
    1000: np.array([
        1.00000, 0.65928, 0.57492, 0.51117, 0.46604, 0.33304, 0.18719,
        0.05702, -0.06080, -0.10648, -0.27805, -0.38289, -0.29730,
        -0.22220, -0.20196, -0.18109, 0.00000
    ]),
}

# x-coordinates for the v-velocity profile (Table II).
GHIA_X = np.array([
    1.0000, 0.9688, 0.9609, 0.9531, 0.9453, 0.9063, 0.8594, 0.8047,
    0.5000, 0.2344, 0.2266, 0.1563, 0.0938, 0.0781, 0.0703, 0.0625, 0.0000
])

# v along horizontal centerline y=0.5
GHIA_V = {
    100: np.array([
        0.00000, -0.05906, -0.07391, -0.08864, -0.10313, -0.16914,
        -0.22445, -0.24533, 0.05454, 0.17527, 0.17507, 0.16077, 0.12317,
        0.10890, 0.10091, 0.09233, 0.00000
    ]),
    400: np.array([
        0.00000, -0.12146, -0.15663, -0.19254, -0.22847, -0.23827,
        -0.44993, -0.38598, 0.05186, 0.30174, 0.30203, 0.28124, 0.22965,
        0.20920, 0.19713, 0.18360, 0.00000
    ]),
    1000: np.array([
        0.00000, -0.21388, -0.27669, -0.33714, -0.39188, -0.51550,
        -0.42665, -0.31966, 0.02526, 0.32235, 0.33075, 0.37095, 0.32627,
        0.30353, 0.29012, 0.27485, 0.00000
    ]),
}

# Primary-vortex stream-function value and location (Ghia Table, |psi|_max).
GHIA_PSI_PRIMARY = {
    100:  {"psi": -0.103423, "x": 0.6172, "y": 0.7344},
    400:  {"psi": -0.113909, "x": 0.5547, "y": 0.6055},
    1000: {"psi": -0.117929, "x": 0.5313, "y": 0.5625},
}
